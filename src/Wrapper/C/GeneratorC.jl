    ...
    if !isempty(enum_types)
        ...
        for enum_key in sort(collect(enum_types))
            ...
            if !isempty(enumerators)
                # Sanitize FIRST, then choose the shape. Both branches consume the
                # same pairs, so the emitted member spelling cannot depend on which
                # one runs — the bitflag path is a different container for identical
                # names and values, not a different naming decision.
                members = Pair{String,Any}[]
                seen_names = Set{String}()
                for enumerator in enumerators
                    name = _escape_keyword(_sanitize_c_type_name(get(enumerator, "name", "Unknown")))
        ...
        for (enum_name, members) in header_enums
            ...
            if !isempty(members)
                ...
                for (member_name, value) in members
                    n = _escape_keyword(_sanitize_c_type_name(string(member_name)))
                ...
                if _is_bitflag_enum(last.(emitted))
                    push!(enum_chunks, _bitflag_enum_chunk(enum_name, underlying, emitted))
                else
                push!(enum_chunks, """
...
$_preload_snippet                # Load library explicitly to ensure symbols are available
    ...
    # Tier-1 registry. Returns an EMPTY STRING when no slices were emitted, so a
    # wrapper built with `[wrap.tier1] enable = false` carries no llvmcall surface
    # at all — no TIER1_FUNCTIONS, no TIER1_DECLARES, no unreachable
    # `_slice_symbols_resolve`, and none of the explanatory comment for a
    # mechanism it does not use. Defined in Tier1C.jl.
    tier1_registry = _tier1_registry_chunk(tier1_emitted, tier1_declares)
