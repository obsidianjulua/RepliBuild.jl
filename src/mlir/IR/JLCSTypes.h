...
struct CStructTypeStorage : public ::mlir::TypeStorage {
  using KeyTy = std::tuple<StringAttr, ::llvm::ArrayRef<Type>, ArrayAttr>;

  CStructTypeStorage(StringAttr juliaTypeName, ::llvm::ArrayRef<Type> fieldTypes, ArrayAttr fieldOffsets)
      : juliaTypeName(juliaTypeName), fieldTypes(fieldTypes), fieldOffsets(fieldOffsets) {}

  bool operator==(const KeyTy &key) const {
    return key == KeyTy(juliaTypeName, fieldTypes, fieldOffsets);
  }
  ...
  static CStructTypeStorage *construct(::mlir::TypeStorageAllocator &allocator, const KeyTy &key) {
    ...
    return new (allocator.allocate<CStructTypeStorage>())
        CStructTypeStorage(juliaTypeName, fieldTypes, fieldOffsets);
  }
