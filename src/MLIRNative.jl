...
function create_jit(mod::MlirModule; opt_level::Int=2, dump_object::Bool=false, shared_libs::Vector{String}=String[])
    if isempty(shared_libs)
        ...
        lib_ptrs = [Base.unsafe_convert(Cstring, s) for s in shared_libs]
        GC.@preserve shared_libs lib_ptrs begin
            jit = ccall((:jlcs_create_jit_with_libs, libJLCS), MlirExecutionEngine,
                        (MlirModule, Cint, Bool, Ptr{Cstring}, Cint),
                        mod, opt_level, dump_object, lib_ptrs, length(lib_ptrs))
        end
    end
    if jit == C_NULL
