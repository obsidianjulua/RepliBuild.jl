...
Varargs ABI + cache-lifecycle release from the 2026-07-10 C-generator audit: variadic calls are now formally correct on x86-64 SysV, and the registry build cache stops serving wrappers generated by outdated codegen.
...
`Project.toml` (stale at 2.5.8), `RepliBuild.VERSION` (stale at 2.5.7), and the `runtests.jl` pin now all say 2.5.10. The fingerprint reads `Project.toml` via `pkgversion`, so keeping it bumped now has teeth.

## v2.5.9

Dialect fix for C++ virtual dispatch: `jlcs.vcall` now translates all the way to LLVM IR instead of segfaulting at emit. Also removes the struct-by-value convenience overloads from both wrapper generators — a crash-proven double-free footgun.
...
Survivors, pinned by test: the `Vector{T}` convenience overload for input-array params (`Ptr{Cdouble}` etc. under `GC.@preserve`) stays, and its `Cstring` returns now follow the **base wrapper's policy** — NULL check + `String` via `unsafe_string` — instead of leaking a raw `Cstring` (previously `cJSON_Print(item::Any)` returned `String` while the by-value overload returned `Cstring`). The surviving path also gains the base wrapper's struct-return sentinel guard on the C++ side (previously it emitted the boxed-`Any` ccall return, a latent segfault).
