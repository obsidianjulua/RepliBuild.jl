...
@testset "BasicsTest Verification" begin
    ...
    # Julia structs always use natural alignment, so sizeof == 8 (not 5 like C packed)
    # The JIT thunk handles the layout conversion between Julia (aligned) and C (packed)
    @test sizeof(PackedStruct) == 8

    process_packed(packed)  # Should print "Packed: a=30, b=40"
